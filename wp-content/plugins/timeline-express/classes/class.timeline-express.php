										...
										?>
											<div class="cd-timeline-block">
												...
												<div class="cd-timeline-content" style="background:<?php if ( $content_background == '' ) { echo 'transparent'; } else { echo $content_background; } ?>;box-shadow: 0 3px 0 <?php if ( $content_shadow == '' ) { echo 'transparent'; } else { echo $content_shadow; } ?>;">
													<!-- title -->
													<span class="cd-timeline-title-container"><h4 class="cd-timeline-item-title"><?php the_title();?></h4><?php if ( $this->timeline_express_optionVal['date-visibility'] == 1 ) { ?>
														<!-- release date -->
														<!-- now localized, for international date formats based on the 'date_format' option -->
														<span class="timeline-date"><h5><?php echo date_i18n( apply_filters( 'timeline_express_custom_date_format' , get_option( 'date_format' ) ) , get_post_meta( $post->ID , 'announcement_date' , true ) ); ?></h5></span>
													<?php } ?></span>
													<?php
